...
### 9. **网页解析的过程与实现方法,    在浏览器中输入URL后执行的全部过程（如[www.baidu.com](http://www.baidu.com)），    在浏览器中输入一个网站，发生的事情（客户端，服务端）（star）**
   **1）用户输入网址，浏览器发起DNS查询请求，获得IP地址：** 用户访问网页，DNS服务器（域名解析系统）会根据用户提供的域名查找对应的IP地址。域名解析服务器是基于UDP协议实现的一个应用程序，通常监听53号端口来获取客户端的域名解析请求。DNS查找过程如下：
   	①查找浏览器缓存；
   	②浏览器做一个系统调用，查找系统缓存；
   	③将查询请求发给路由器，查找路由器缓存；
...
​	系统调用时用户进程和硬件设备之间的**中间层**，是用户访问内核的**唯一手段**。
 	...
 	当程序中有系统调用语句，程序执行到系统调用时，首先**使用类似int 80H的软中断指令**，保存现场，取得系统调用号，在内核态执行调用，然后**恢复现场**，每个进程都会有两个栈，一个内核态栈和一个用户态栈，其信息（ss0,esp0 & ss,esp存于`task_struct`）。当执行int中断执行时就会由用户态，栈转向内核栈。系统调用时需要进行栈的切换。而且内核代码对用户不信任，**需要进行额外的检查**。系统调用的返回过程有很多额外工作，比如**检查是否需要调度**等。

**<font color=red>（有待验证，不确定※）</font>**同时进入内核态时cr3会切换，即页表会切换到内存页表，**TLB会刷新**，和进程切换一样，会导致变慢。

**具体过程：**

  （1）为系统调用设置参数，**将系统调用号保存到eax寄存器中**。然后触发INT $0x80开始执行异常处理程序

  （2）系统调用处理程序从当前进程的描述符`task_struct`中提取其**内核栈**的ss0及esp0（栈顶）信息（在`TSS`中），使用ss0和esp0替换寄存器，使其指向的内核栈，进入内核态。

  （3）首先，把异常处理程序可以用到的所有CPU寄存器保存到内核栈中（通过`save_all`可以存5个参数，如果更多就传递指针，通过copy_from_user函数在指针处获取参数 ），将当前进程PCB地址存放到ebp中，然后**根据eax中的系统调用号在系统调用表**中调用对应的服务程序，同时保存了被暂停执行的程序的下一条指令，将程序已至挂起队列
...
​	在此基础上，**进程切换还需要**保存进程的**私有资源**（如虚拟内存（程序代码段，数据段，用户堆栈段，内核信息）），而**线程切换（一般认为是同一进程的两个线程）需要保存线程间的私有资源（如线程栈和相关寄存器状态,PC）。**
...
​		**1. 把当前上下文存入栈里（程序计数器，各种寄存器）**
​		**2. 切换页目录以使用新的线性地址空间**

​		**3. 刷新页表缓冲（TLB）=>导致进程切换开销的** 【内核态用户态切换也有此开销】

​		**4. 切换内核态堆栈和硬件上下文（通过弹栈）**
...
同一进程的不同线程有独立的task_struct，但共享同一个mm_struct。
...
### 26. **僵尸进程问题（可以设置sigchld的函数为wait，由init去收尸，或双fork） 孤儿进程 守护进程**
  ...
  **僵尸进程：**(star)
  ...
  ​	在每个进程退出的时候，内核释放该进程所有的资源，包括打开的文件，占用的内存。但是仍然保留了一些信息（如进程号pid 退出状态 运行时间等）。这些保留的信息直到进程通过调用wait/waitpid时才会释放。也就是说，僵尸进程存在的目的是为了**让父进程得到子进程的退出信息**
...
1. 检查该虚拟地址是否合法，与每个vm_area_struct对比：不合法则段错误
2. 检查进程访问是否合法，即进程是否拥有读写权限
3. 若内存中有空闲块，则直接调入
4. 否则根据页面置换算法选择一个牺牲页，将它交换出去，如果该页发生过修改则进行回写，然后换入新的页面并更新页表。
5. 当缺页处理程序返回时，CPU重新启动引起缺页的指令
