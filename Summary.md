...
① 执行可执行文件，加载器将可执行文件的.text 、.data和.rodata拷贝，填充.bss
...
**答**：可以，同一个端口虽然udp和tcp的端口数字是一样的，但实质他们是不同的端口，所以是没有影响的，从底层实质分析，对于每一个连接内核维护了一个五元组，包含了源ip，目的ip、源端口目的端口、以及传输协议，在**这里尽管前4项都一样，但是传输协议是不一样的**，所以内核会认为是2个不同的连接，在ip层就会进行开始分流，tcp的走tcp，udp走udp。
...
epoll只有在epoll_ctl的时候把注册的fd从用户空间复制到内核空间，在每次epoll_wait的时候把活跃事件数从内核空间复制到用户空间，故epoll_wait的复杂度为O(1)。（而poll则是每次调用poll都会将所有监听的fd从用户复制到内核，返回时又从内核复制到用户，轮询）。
...
**fork实现：**
fork就是简单的把父进程的几乎所有地址空间和内核部分都**拷贝**一份，先**通过slab创建**task_struct 、thread_info 、内核栈，然后**通过dup_mmap复制**父进程的地址空间地址结构（**先通过复制页表暂时共享这个页面，到子进程（或父进程）真的要写这个页面时再次分配页面和复制**），然后**修改**一些标志让自己与父进程变得不一样（初始化），分配得到一个有效PID。由于**写时复制技术**，实际开销就是创建并复制父进程的页表以及给子进程创建一个进程描述符。
...
**具体过程：**
  ...
  （2）系统调用处理程序从当前进程的描述符`task_struct`中提取其**内核栈**的ss0及esp0（栈顶）信息，使用ss0和esp0替换寄存器，使其指向的内核栈，进入内核态。

  （3）首先，把异常处理程序可以用到的所有CPU寄存器保存到内核栈中（通过save_all可以存5个参数，如果更多就传递指针，通过copy_from_user函数在指针处获取参数 ），将当前进程PCB地址存放到ebp中，然后**根据eax中的系统调用号在系统调用表**中调用对应的服务程序，同时保存了被暂停执行的程序的下一条指令，将程序已至挂起队列
...
2. **存储数据**
    根据需要的磁盘大小，在数据块位图寻找合适数量的数据块，并把数据块位图对应位置1
3. **添加文件名到目录**
