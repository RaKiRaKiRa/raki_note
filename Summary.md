...
![image.png](Summary/1596736513399-355fa821-5d12-4e27-afb2-3c5d70273d89.png)
...
**<font color=red>（有待验证，不确定※）</font>**同时进入内核态时cr3会切换，即页表会切换到内存页表，**TLB会刷新**，和进程切换一样，会导致变慢。
...
1. **寻找inode**
    内核根据inode位图先找到一个空闲的inode，把文件信息记录到其中，并把inode位图对应位置1
2. **存储数据**
    根据需要的磁盘大小，在数据块位图寻找合适数量的数据块，并把数据块位图对应位置1，并将其信息添加进inode
3. **添加文件名到目录**
    键入新的文件名abc，内核将入口（ 131074，abc）添加到其目录文件，文件名和 inode 之间的对应关系将文件名和文件的内容及属性连接起来
...
​	1.因为**进程拥有独立的堆栈空间和数据段**，所以每当启动一个新的进程必须分配给它**独立的地址空间**，建立众多的数据表来维护它的代码段、堆栈段和数据段，系统开销比较大，而线程不一样，**线程拥有独立的堆栈空间，但是共享数据段，它们彼此之间使用相同的地址空间**，共享大部分数据，而同一进程的线程有**独立的程序计数器（下一条指令）、寄存器（当前变量）和堆栈（执行历史）**。**线程切换速度也比进程快**，效率高，但是正由于进程之间独立的特点，使得进程安全性比较高。也因为进程有独立的地址空间，一个进程崩溃后，在保护模式下不会对其它进程产生影响，而线程只是一个进程中的不同执行路径。**一个线程死掉就等于整个进程死掉。**
​	2.体现在通信机制上面，正因为**进程之间互不干扰，相互独立**，进程的通信机制相对很复杂，譬如管道，信号，消息队列，共享内存，套接字等通信机制，而**线程由于共享数据段所以通信机制很方便**。
​	3.属于同一个进程的**所有线程共享该进程的所有资源**，包括文件描述符。而不同过的进程相互独立。

​		 clone(CLONE_VM | CLONE_FS | CLONE_FILES |CLONE_SIGHAND, 0)

​	线程使得CPU系统更加有效，操作系统会保证当线程数不大于CPU数目时，不同的线程运行于不同的CPU上。

​	**同一进程的不同线程有独立的`task_struct`，共享同一个mm_struct，但sp指向的vma_area_struct不同(即拥有独立的线程栈）（CLONE_VM控制）**
