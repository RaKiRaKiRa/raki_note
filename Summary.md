...
​	系统调用时用户进程和硬件设备之间的中间层，是用户访问内核的**唯一手段**。
  ...
  **具体过程：**
  ...
  （1）为系统调用设置参数，其中一个参数是系统调用编号，然后触发INT $0x80开始执行异常处理程序，即系统调用处理程序，准备切换到内核态

  （2）系统调用处理程序从当前进程的描述符中提取其内核栈的ss0及esp0信息。

  （3）使用ss0和esp0指向的内核栈将当前进程的cs,eip,eflags,ss,esp等用户信息保存起来，这个过程也**完成了由用户栈到内核栈的切换过程**，同时保存了被暂停执行的程序的下一条指令，将程序已至挂起队列。

  （4）将先前由中断向量检索得到的中断处理程序的cs,eip信息装入相应的寄存器，ebx, ecx, edx, esi和edi按照顺序存放前五个参数，开始执行中断处理程序，这时就转到了内核态的程序执行了。

  （5）恢复用户程序状态，将程序放入已就绪队列。
